...
async def clear_all_force_sub_channels(chat_id: int):
...
async def set_force_sub_channel_label(chat_id: int, label: str, channel: str):
    """Add a labelled channel to force-sub (label stored for display, channel for checking)."""
    await db.force_sub.update_one(
        {"chat_id": chat_id},
        {
            "$addToSet": {"channels": channel},
            "$set": {f"labels.{channel.lstrip('@').replace('-', '_')}": label},
        },
        upsert=True,
    )


async def get_force_sub_labels(chat_id: int) -> dict:
    """Return {channel: label} mapping for a chat."""
    data = await db.force_sub.find_one({"chat_id": chat_id})
    if data:
