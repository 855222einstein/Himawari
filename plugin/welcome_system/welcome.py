...
def register_welcome_system(app):
    ...
    async def toggle_welcome(client, message):
        ...
        if option == "on":
            ...
            return await message.reply_text("ᴡᴇʟᴄᴏᴍᴇ ᴇɴᴀʙʟᴇᴅ.")

        elif option == "off":
            await db.set_welcome_status(message.chat.id, False)
            return await message.reply_text("ᴡᴇʟᴄᴏᴍᴇ ᴅɪꜱᴀʙʟᴇᴅ.")
    ...
    async def set_welcome(client, message):
        ...
        if len(text) < 2:
            return await message.reply_text(
                "ᴜꜱᴀɢᴇ\n"
                "/setwelcome ᴡᴇʟᴄᴏᴍᴇ {first_name}"
            )

        await db.set_welcome_message(message.chat.id, text[1])
        await message.reply_text("ᴡᴇʟᴄᴏᴍᴇ ᴍᴇꜱꜱᴀɢᴇ ᴜᴘᴅᴀᴛᴇᴅ.")
